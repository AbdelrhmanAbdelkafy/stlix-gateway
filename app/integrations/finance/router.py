...
config rather than a constant, but it must not be switched to live without an
explicit owner decision and a passing automated reconciliation guard.
...
def _source(explicit: str | None, settings: Settings) -> str:
    ...
    The default is configuration, not a constant. It remains SQL until the owner
    explicitly approves live after the automated reconciliation guard passes.
    """
